...
function TableTree4J(objectName,tableTree4JDir){
	...
	//add grid node
	TableTree4J.prototype.addGirdNode=function(dataList,id,pid,booleanOpen,order,url,target,hrefTip,hrefStatusText,classStyle,icon,iconOpen,dataResList){
		var nod=new Node(dataList,id,pid,booleanOpen,order,url,target,classStyle,icon,iconOpen,hrefTip,hrefStatusText,dataResList);
		this.addNode(nod);
	}
		...
		if(node.dataList.length>1){
			...
			if(node.booleanRoot==true&&this.headerWidthList!=null){
				...
				for(var i=1;i<node.dataList.length;i++){
					...
					str=str+"<td name=\""+node.dataResList[0]+"\" id="+node.dataResList[1]+" value="
					+node.dataResList[i+1]+" "+clostyleMark+" onClick=\"getEventElement(event,'"+node.id+"');\">"+node.dataList[i]+"</td>"
					clostyleMark="";
				}
			}
			str=str+"</tr>";
	...
	//open all node
 	...
 	TableTree4J.prototype.openRootNode=function(){
		clickNode(this.rootNode.id,this);
	}
 	TableTree4J.prototype.closeRootNode=function(){
		clickNode(this.rootNode.id,this);
	}
	...
	TableTree4J.prototype.initRootNode=function(){
		...
		if(allNodes.length>0){
			...
			if(rootNod.childNodes && rootNod.childNodes.length==0){
				dImgcode="";
			}
...
//Data Value Object
function Node(dataList,id,pid,booleanOpen,order,url,target,classStyle,icon,iconOpen,hrefTip,hrefStatusText,dataResList){
...
function getEventElement(event,nodeId){
}